...
def sha512(input_file):
    ...
    bv = BitVector(textstring = input_open.read())
    bv_add1 = bv + BitVector(bitstring = "1")
    zeros = ((1024-128)- bv_add1.length()) % 1024
    list_of_zero = [0] * zeros

    bv_final = (bv_add1 + BitVector(bitlist = list_of_zero)) + BitVector(intVal = bv.length(), size=128 )
    m = 0
    for n in range(0, bv_final.length(), 1024):
        input_block = bv_final[n:n+1024]

        words[0:16] = [input_block[i:i+64] for i in range(0,1024,64)]
...
if __name__ == "__main__":
   sha_hash_value  = sha512(sys.argv[1])
   output_file = open(sys.argv[2], 'w')
   output_file.write(sha_hash_value)
